...
import { Bell, Info, CheckCircle, Dot, Check } from "lucide-react";
...
const Notifications = () => {
  ...
  try {
    await axios.put(`/api/users/notifications/${id}/read`, null, {
            ...
            {notifications.map((notif) => (
              <li
                ...
                className={`relative bg-white border border-gray-200 rounded-lg shadow-sm p-4 flex gap-3 items-start transition-all duration-300 ${
                  !notif.read ? 'hover:bg-gray-50 cursor-pointer' : 'bg-gray-50'
                }`}
              >
